...
def main():
    ...
    for epochs, batch_size, layer_units in config.network_archs:
        ...
        for it in range(config.num_iter):
            ...
            job_name = '{}_TEST_JOB'.format(model_string)
            with open('jobs/{}/{}'.format(config.dataset_name, job_name), 'w') as job_file:
                job_file.write('#PBS -e jobs/{f}/{j}.test.e.txt -o jobs/{f}/{j}.test.o.txt -l nodes=1:ppn=2\n'.format(f=config.dataset_name, j=job_name))
                job_file.write('cd $PBS_O_WORKDIR\n')
                job_file.write('python test.py --dataset {d} --batch-size {bs} --layer-units {lu} --cpu --iter {i}\n'.format(
